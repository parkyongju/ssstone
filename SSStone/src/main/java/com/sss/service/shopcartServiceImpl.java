...
public class shopcartServiceImpl implements shopcartService {
	...
	public boolean insert(Shopping_cartVO vo) {
		...
		for (Shopping_cartVO i : cart) {
			if (vo.getP_no().toString().equals(i.getP_no().toString())) {
				if (vo.getS_amount() + i.getS_amount() > 5) {
					return false;
				} else {
					long amount = (int) (vo.getS_amount() + i.getS_amount());
					Shopping_cartVO updatevo = new Shopping_cartVO();
					updatevo.setS_amount(amount);
					updatevo.setS_no(i.getS_no());
